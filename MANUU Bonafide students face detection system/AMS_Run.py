...
window = tk.Tk()
window.title("Bonafide Students Face Detection System")
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        DB_table_name = str(subb + "_Manual_" + Date)
...
def Fillattendances():
        ...
        import tkinter
        root = tkinter.Tk()
        root.title("Auto bonafide students attendance")
        root.configure(background='grey80')
        sub = "Auto"
        now = time.time()  # For calculate seconds of video
        future = now + 5
        Notifica = tk.Label(root, text="Attendance filled Successfully", bg="Green", fg="white", width=33, height=2, font=('times', 15, 'bold'))
        if time.time() < future:
            if sub == '':
                err_screen1()
            else:
...
message = tk.Label(window, text="MANUU Bonafide Students Face Detection System", bg="black", fg="white", width=45,
...
FA = tk.Button(window, text="Start", fg="black", command=Fillattendances,
               bg="SkyBlue1", width=20, height=3, activebackground="white", font=('times', 16, ' bold '))
